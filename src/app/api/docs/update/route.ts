...
import { updateResource, updateSaveDocs } from "@/services/api/common-erpnext-api/create-edit-api";

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();

    const { doctype, name, updateData } = body;
    const token = req.headers.get("Authorization");


    if (!doctype || !name || !updateData || !token) {
    ...
    let result = await updateSaveDocs(doctype, name, updateData, token);
    console.log("update result here",result);
    if (result?.error) {
