...
export async function POST(req: NextRequest) {
  try {
    ...
    // Example payload calling your custom method in Frappe
    const bodyParams = {
      version: CONSTANTS.VERSION,
      method: "get_variant_details", // <-- your custom method
      entity: "product",
      item_code,
    };
    ...
    if (!frappeRes.ok) {
      return NextResponse.json({ error: `ERPNext request failed with status ${frappeRes.status}` }, { status: frappeRes.status });
    }
