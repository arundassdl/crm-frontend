...
import TaskListWithCreate from "@/components/Deals/Detail/TaskListWithCreate";

const LeadDetailTabs = ({customerData}) => {
  const [activeTab, setActiveTab] = useState(0); // Manage active tab
   console.log("customerData herrrre",customerData);
   

const tabsData = [
  { label: "Overview", icon: <InfoIcon />, Component: <LeadOverview detail={customerData} /> },
  { label: "Tasks", icon: <TaskAltIcon />, Component: <TaskListWithCreate link_name={customerData?.record?.name} doc_type="CRM Lead"  /> },
  // { label: "Notes", icon: <NoteIcon />, Component: <Notes /> },
  { label: "Attachments", icon: <AttachFileIcon />, Component: <Attachments slug={customerData} /> },
