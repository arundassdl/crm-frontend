...
import { submit_adduser } from "@/services/api/users/users-api";
...
const AddUser: any = () => {
    ...
    useEffect(() => {
        ...
        console.log("installation list", window?.location?.origin);
    ...
    const handlesubmit: any = async (values: any, action: any) => {
        ...
        let userApiRes: any = await submit_adduser(formData, userToken?.access_token);
                    ...
                    <Grid item xs={12} md={12} lg={12}>
                        ...
                        <div className="flex flex-wrap justify-between max-sm:flex-col sm:items-center gap-x-6 gap-y-4">
                            <div className="flex flex-col items-start gap-2">
                                <Typography variant='h4'>{(selectedMultiLangData?.add_user) ? selectedMultiLangData?.add_user : "Add User"}</Typography>
                            </div>
                            {userData?.userType != "Customer" && userData?.userType != "Surveyor" ?
                                (
                                    <>
                                        <div className='flex items-center justify-between flex-wrap gap-3'>
                                            <Button variant="outlined" href={`/jobs`} startIcon={<i className="ri-arrow-go-back-line" />} size="medium">
