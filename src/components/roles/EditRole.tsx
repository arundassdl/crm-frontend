...
import { fetchpermissionmodule, submit_editroles , get_roleby_name} from "../../services/api/roles/roles-api";
...
const EditRole: any = (slug) => {
    ...
    const handlesubmit: any = async (values: IValue, action: any) => {
        ...
        // // return false;
        let EditRoleApiRes: any = await submit_editroles(formData, userToken?.access_token);
        setTimeout(() => {
            console.log('here', EditRoleApiRes);
            if (EditRoleApiRes?.msg === "success") {
                showToast(EditRoleApiRes?.data.msg, "success");
                router.push("/roles");
            } else {
                showToast(EditRoleApiRes?.error, "error");
            }    
        }, 1000);
                    ...
                    <Grid item xs={12} md={12} lg={12}>
                        ...
                        <div className="flex flex-wrap justify-between max-sm:flex-col sm:items-center gap-x-6 gap-y-4">
                            <div className="flex flex-col items-start gap-2">
                                <Typography variant='h5'>{(selectedMultiLangData?.edit_role) ? selectedMultiLangData?.edit_role : "Edit Role"}</Typography>
                            </div>
                                <>
                                                                                                                    ...
                                                                                                                    <>
                                                                                                                        {historyRow?.category_order_sequence ==  data.order_sequence  ? (
                                                                                                                            <TableRow key={historyRow.order_sequence} sx={{ '& > *': { borderBottom: 'unset' } }}>
                                                                                                                                ...
                                                                                                                                <TableCell width="5%" component="td"> <Checkbox id={`permission.${historyRow?.order_sequence}.cimport `} checked={historyRow?.cimport}  onChange={handleChange} /></TableCell>
                                                                                        ...
                                                                                        <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
                                                                                            <TableCell style={{ paddingBottom: 0, paddingTop: 0, paddingLeft: 0,  paddingRight: 0 }} colSpan={10}>
                                                                                                <TableRow key={data.order_sequence} sx={{ '& > *': { borderBottom: 'unset' } }}>
                                                                                                    ...
                                                                                                    <TableCell width="5%" component="td"> <Checkbox id={`permission.${data?.order_sequence}.cimport `} checked={data?.cimport}  onChange={handleChange} /></TableCell>
