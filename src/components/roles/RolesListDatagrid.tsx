...
import { getRolesListDatagrid } from "../../services/api/roles/roles-api";
...
export default function RolesList(props) {
  ...
  let [rolesList, setRolesList] = useState<any>([]);
  let [rolesCount, setRolesCount] = useState<Number>(0);
  ...
  const handleActivate: any = async (user: any) => {
    ...
    request['enabled'] = (user?.enabled == 1) ? 0 : 1
    user["enabled"] = request['enabled']
    // const ActivatePartner: any = await ActivateDeactivateSalePartner(
    //   request, TokenFromStore?.token
    // );
    // if (ActivatePartner?.msg === "success") {
    //   showToast(ActivatePartner?.data.msg, "success");
    // } else {
    //   showToast(ActivatePartner?.error, "error");
    // }
  ...
  const columns: GridColDef[] = [
    ...
    {
        field: 'disabled', headerName: "Active", width: 100, type: 'singleSelect', valueOptions: [0, 1], headerClassName: "sdl-tbl--header"
        , renderCell: (params) => {
          let inst_status_colr = 'inherit'
            const inst_statusIcn: instStatusIcn[] = []
            if (params?.row?.disabled == '0' || params?.row?.disabled == 0) {
            ...
            } else if (params?.row?.disabled == '1' || params?.row?.disabled == 1) {
    ...
    {
      field: 'created_date', headerName: "Created Time", width: 120,
      // valueGetter: (value, row) => value ?(new Date(value), `dd-LL-yyyy`) : "N/A",
      type: 'date', renderCell: (params) => {
        let day = new Date(params?.row?.creation.toLocaleString()).getDate() < 10 ? "0" + new Date(params?.row.creation.toLocaleString()).getDate() : new Date(params?.row.creation.toLocaleString()).getDate();
        let month = new Date(params?.row?.creation.toLocaleString()).getMonth() < 9 ? "0" + (new Date(params?.row?.creation.toLocaleString()).getMonth() + 1) : (new Date(params?.row.creation.toLocaleString()).getMonth() + 1);
        let year = new Date(params?.row.creation.toLocaleString()).getFullYear()
        let creation = day + "/" + month + "/" + year
        return (creation)
      }
    },
    {
      field: 'modified_date', headerName: "Modified Time", width: 130, type: 'date', renderCell: (params) => {
        if (params?.row?.modified != "null" && params?.row?.modified != null && params?.row?.modified != '') {
          // return (<ReactTimeAgo date={params?.row?.modified} locale="en-US" />)
          let day = new Date(params?.row?.modified.toLocaleString()).getDate() < 10 ? "0" + new Date(params?.row.modified.toLocaleString()).getDate() : new Date(params?.row.modified.toLocaleString()).getDate();
          let month = new Date(params?.row?.modified.toLocaleString()).getMonth() < 9 ? "0" + (new Date(params?.row?.modified.toLocaleString()).getMonth() + 1) : (new Date(params?.row.modified.toLocaleString()).getMonth() + 1);
          let year = new Date(params?.row.modified.toLocaleString()).getFullYear()
      ...
      getActions: (params) => {
        ...
        const actionData = [
          <GridActionsCellItem
            ...
            label="View"
            onClick={(event) => {
              handleDetailPageClick(params?.row?.role_name);
            }}
            showInMenu
            className="gap-4"
          />,];
          if (params?.row?.is_admin == 0) {
            actionData.push(<GridActionsCellItem
              ...
              label="Edit"
              onClick={(event) => {
                handleEdit(params?.row?.role_name)
              }}
              showInMenu
              className="gap-4"
            />,)
          ...
          if (params?.row?.is_admin == 0) {
            actionData.push(
              <GridActionsCellItem
                icon={<Switch aria-label="Activate" defaultChecked={(params?.row?.disabled) ? false : true} size="medium" color="success" className="gap-2" sx={{ marginLeft: "-5px" }} />}
                label={(params?.row?.disabled) ? "Deactivate" : "Activate"}
                onClick={(event) => {
  ...
  const rows = (rolesList?.list?.length > 0) ? rolesList?.list : [];
  ...
  const fetchFilteredData = async (page, rowsPerPage, sortModel, filterModel) => {
    ...
    try {
      console.log('inside try before get roleslistdatagrid')
      const RolesListss: any = await getRolesListDatagrid(filterModel, TokenFromStore?.token);
      console.log('RolesListss>>>>', RolesListss)
      setRolesList(RolesListss?.list || []);
      setRolesCount(RolesListss?.count || 0);
    } catch (error) {
        ...
        <CardContent>*/}
          ...
          > 
            <StyledDataGrid
              ...
              slotProps={{ toolbar: { userData: userData, module: "Role", title:(selectedMultiLangData?.menu_sales_partner) ? selectedMultiLangData?.menu_sales_partner : "Roles", total:Number(rolesCount)  } as ExtendedGridToolbarProps }}
              loading={loading}
            />
           </Box>
        {/*</CardContent>
