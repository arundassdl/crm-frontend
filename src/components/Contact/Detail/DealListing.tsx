...
const DealListing: React.FC<DealListingProps> = ({ link_name }) => {
  ...
  //     setFilterModel({
  //       items: [
  //         { field: 'CRM Contacts.contact', operator: 'equals', value: link_name },
  //         { field: 'CRM Contacts.parenttype', operator: 'equals', value: "CRM Deal" }
  //       ],
  //       quickFilterValues: []
  //     });
  //   }
  ...
  const fetchData = async () => {
    ...
    if (link_name != "" && link_name != undefined) {
      ...
      const relatedDocs = ["CRM Contacts"];
       const Joinquery = "LEFT JOIN `tabCRM Contacts` ON `tabCRM Contacts`.`parent` = `tabCRM Deal`.`name`";
        const whereconditon =" AND `tabCRM Contacts`.`contact` = '"+link_name+"'";
