...
export default function CommonSearchField({
  ...
  const fetchOptions = async () => {
    ...
    if(debouncedValue==undefined || inputValue==undefined){
      setInputValue("");
    }
    if(inputValue==undefined){
      setInputValue("");
    }
    console.log("queryqueryquery inputValue", inputValue);
    setLoading(true);
  ...
  useEffect(() => {
    ...
    const fetchData: any = async (name) => {
      ...
      let getRecords = await fetchDetailData(
        ...
        console.log("get cont Records",getRecords?.record);
        console.log("getRecords?.record?.length",Object.keys(getRecords).length);
        setUserData(getRecords?.record)
        onUserSelect(getRecords?.record)
        // if(getRecords?.record?.length)
        // await onCustomerSelect(getRecords?.record);
        }
      }
  useEffect(() => {
