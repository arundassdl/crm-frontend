...
interface TasksKanbanBoradProps {
    tasks: Task[];
...
const TasksKanbanBorad: React.FC<TasksKanbanBoradProps> = ({
    tasks,
    statusOptions,
    userData,
    onStatusChange,
    ...
    const onDragEnd = async (result: DropResult) => {
        ...
        if (res.ok) {
            console.log(`Task ${draggableId} updated successfully.`);
        } else {
            console.error(`Failed to update Task ${draggableId}`);
        }
    };

    return (
        <DragDropContext onDragEnd={onDragEnd}>
            <Box sx={{ display: "flex", overflowX: "auto", gap: 2, p: 2, pl: 3, pt: 5}}>
...
export default TasksKanbanBorad;
