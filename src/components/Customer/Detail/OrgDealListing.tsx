...
interface OrgDealListingProps {
  // contact_list: any;
  link_name: string;
}

const OrgDealListing: React.FC<OrgDealListingProps> = ({ link_name }) => {
  ...
  //     setFilterModel({
  //       items: [
  //         // { field: 'organization', operator: 'equals', value: link_name },
  //         // { field: 'parenttype', operator: 'equals', value: "CRM Deal" }
  //       ],
  //       quickFilterValues: []
  //     });
  //   }
  ...
  const handleDelete = async (id: any) => {
    setDialogOpen(false);
    let deleteRecord = await deleteClient("CRM Deal", id, userToken?.access_token);
  ...
  const handleEditRow = async (rowData: any) => {
    ...
    let getRecords = await fetchDetailData("CRM Deal", rowData?.name, userToken?.access_token);
  ...
  const colFields = [
    ...
    { doctype: module, field: "annual_revenue", hidden: "False" },
  ...
  const fetchData = async () => {
    ...
    if (link_name != "" && link_name != undefined) {
      ...
      const relatedDocs = ["CRM Organziation"];
      const Joinquery = "LEFT JOIN `tabCRM Organization` ON `tabCRM Organization`.`name` = `tabCRM Deal`.`organization`";
      const whereconditon =" AND `tabCRM Deal`.`organization` = '"+link_name+"'";
      const data = await fetchRawDataGridListing(
        ...
        "CRM Deal",
        relatedDocs,
        "", "" ,[], Joinquery, whereconditon
      );
      console.log("contact_list data", data);
  ...
  return (
    <>
      <Box sx={{ height: "50vh", display: "flex", flexDirection: "column" }}>
        <Box sx={{ flexGrow: 1, overflow: "auto" }}>
          ...
          <StyledDataGrid
            ...
            slotProps={{
              toolbar: {
                title: selectedMultiLangData?.deal_list || "Deals",
      ...
      {/* {(open) && (
        <AddEditConctactDrawer initialValues={selectedRow} link_name={link_name} open={open} onClose={CloseDialog} operation={operation} onSubmit={fetchData} />
      )} */}
